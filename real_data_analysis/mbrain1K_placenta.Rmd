...
Distribution plot
```{r}
tiff(filename ="~/Google Drive/Hallu/codes/ckgroup/DropletUtils/manuscript/Rplots/mbrain1K_C6_D%03d.tif",width=10,height=5,units="cm",res=300,pointsize=10, compression = "lzw")
Htmap(mbrain1K_r[sort(filter_mr(n1)[1:100]),colnames(mbrain1K_sub1)],"mbrain1K",Plot = "D", AddBackground = F)
dev.off()

tiff(filename ="~/Google Drive/Hallu/codes/ckgroup/DropletUtils/manuscript/Rplots/mbrain1K_C6_H.tif",width=15,height=15,units="cm",res=300,pointsize=10, compression = "lzw")
...
#Remaining 4 data, novel subpopulation
```{r}
# DATA_SCluster1 <- list()
# DATA_SCluster1[[1]] <- Unique_percent #Alz
# DATA_SCluster1[[2]] <- Unique_percent #mbrain1K
# DATA_SCluster1[[3]] <- Unique_percent #placenta
# DATA_SCluster1[[4]] <- Unique_percent #PBMC8K
#names(DATA_SCluster1) <- c("Alz","mbrain1K","placenta","PBMC8K")
#save(DATA_SCluster1,file="~/Google Drive/Hallu/codes/ckgroup/DropletUtils/4data_SeuratCluster.RData")
load("~/Google Drive/Hallu/codes/ckgroup/DropletUtils/4data_SeuratCluster.RData")
lapply(DATA_SCluster1,function(x) sum(x>=0.8))
...
# Revision: Add background barcodes and see how tSNE looks like

Sample 500 from background barcodes and add to normalized cell matrix. 
Redo PCA and tSNE using Seurat.
...
set.seed(SEED)
BG_500 <- sample(setdiff(colnames(mbrain1K_r),colnames(mbrain1K_f)),500)
BG_mat1 <- mbrain1K_r[,BG_500]


mbrain1K_S <- CreateSeuratObject(counts = cbind(com_mat1,Ent_mat1,ED_mat1,BG_mat1), project = "mbrain1K")
mbrain1K_S$new.ident <- c(rep("Common",ncol(com_mat1)),rep("CB2",ncol(Ent_mat1)),
                          rep("ED",ncol(ED_mat1)),rep("Background",ncol(BG_mat1)))
set.seed(SEED)
mbrain1K_S <- FindVariableFeatures(mbrain1K_S, selection.method = "vst", nfeatures = 4000)
#str(VariableFeatures(mbrain1K_S))
all.genes <- rownames(mbrain1K_S)
...
mbrain1K_S <- RunPCA(mbrain1K_S, features = VariableFeatures(object = mbrain1K_S))
set.seed(SEED)
mbrain1K_S <- RunTSNE(mbrain1K_S, dims = 1:50, check_duplicates = FALSE)
DimPlot(mbrain1K_S, reduction = "tsne", group.by = "new.ident")
...
set.seed(SEED)
BG_500 <- sample(setdiff(colnames(placenta_r),colnames(placenta_f)),500)
BG_mat1 <- placenta_r[,BG_500]


placenta_S <- CreateSeuratObject(counts = cbind(com_mat1,Ent_mat1,ED_mat1,BG_mat1), project = "placenta")
placenta_S$new.ident <- c(rep("Common",ncol(com_mat1)),rep("CB2",ncol(Ent_mat1)),
                          rep("ED",ncol(ED_mat1)),rep("Background",ncol(BG_mat1)))
set.seed(SEED)
placenta_S <- FindVariableFeatures(placenta_S, selection.method = "vst", nfeatures = 4000)
#str(VariableFeatures(placenta_S))
...
placenta_S <- RunPCA(placenta_S, features = VariableFeatures(object = placenta_S))
set.seed(SEED)
placenta_S <- RunTSNE(placenta_S, dims = 1:50, check_duplicates = FALSE)
DimPlot(placenta_S, reduction = "tsne", group.by = "new.ident")
```