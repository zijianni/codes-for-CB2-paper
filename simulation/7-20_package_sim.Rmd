...
SIM_HARD <-
  function(n = 5,
    ...
    if(ED_knee){
      ...
      HARD_threshold <- retain_temp * knee_prop
...
# Check the performance of simple hard threshold

```{r}

######################
Prop <- 10^(seq(0,-2,-0.25))

set.seed(SEED)
mbrain_HARD <- list()
for(i in seq_along(Prop)){
    mbrain_HARD[[i]] <- SIM_HARD(dat = mbrain_r, ED_knee = T,knee_prop = Prop[i])
}

set.seed(SEED)
MALT_HARD <- list()
for(i in seq_along(Prop)){
   MALT_HARD[[i]] <- SIM_HARD(dat = MALT_r, ED_knee = T,knee_prop = Prop[i])
}

set.seed(SEED)
PBMC8K_HARD <- list()
for(i in seq_along(Prop)){
   PBMC8K_HARD[[i]] <- SIM_HARD(dat = PBMC8K_r, ED_knee = T,knee_prop = Prop[i])
}

set.seed(SEED)
mbrain1K_HARD <- list()
for(i in seq_along(Prop)){
   mbrain1K_HARD[[i]] <- SIM_HARD(dat = mbrain1K_r, ED_knee = T,knee_prop = Prop[i])
}

set.seed(SEED)
Alz_HARD <- list()
for(i in seq_along(Prop)){
   Alz_HARD[[i]] <- SIM_HARD(dat = Alz_r, ED_knee = T,knee_prop = Prop[i])
}
...
T293_HARD <- list()
for(i in seq_along(Prop)){
   T293_HARD[[i]] <- SIM_HARD(dat = T293_r, ED_knee = T,knee_prop = Prop[i])
}

set.seed(SEED)
PBMC4K_HARD <- list()
for(i in seq_along(Prop)){
   PBMC4K_HARD[[i]] <- SIM_HARD(dat = PBMC4K_r, ED_knee = T,knee_prop = Prop[i])
}
...
jurkat_HARD <- list()
for(i in seq_along(Prop)){
   jurkat_HARD[[i]] <- SIM_HARD(dat = jurkat_r, ED_knee = T,knee_prop = Prop[i])
}
...
PanT4K_HARD <- list()
for(i in seq_along(Prop)){
   PanT4K_HARD[[i]] <- SIM_HARD(dat = PanT4K_r, ED_knee = T,knee_prop = Prop[i])
}
...
PBMC33K_HARD <- list()
for(i in seq_along(Prop)){
   PBMC33K_HARD[[i]] <- SIM_HARD(dat = PBMC33K_r, ED_knee = T,knee_prop = Prop[i])
}


save(MALT_HARD,mbrain_HARD,mbrain1K_HARD,PBMC33K_HARD, PBMC4K_HARD,PBMC8K_HARD,Alz_HARD,
     T293_HARD,jurkat_HARD,PanT4K_HARD,
     file = "~/Google Drive/Hallu/codes/ckgroup/DropletUtils/sim/sim_2_5_HARD_Threshold.RData")
...
for(i in seq_along(HARD_list)){
    HARD_power[,i] <- unlist(lapply(HARD_all[[i]],function(x) x$Stat[1,1]))
    HARD_FDR[,i] <- unlist(lapply(HARD_all[[i]],function(x) x$Stat[2,1]))
}
...
## Lower the upper threshold in real data (PBMC8K, mbrain1K, placenta)
```{r}
