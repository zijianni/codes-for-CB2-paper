...
R version 3.6, Bioconductor version 3.9 and DropletUtils version 1.3.14

Delete protein in MALT and PBMC1K at the beginning.
...
# MALT_r <- as(MALT_r, "dgCMatrix")
# MALT_f <- as(MALT_f, "dgCMatrix")
# 
# mbrain_r <- as(mbrain_r, "dgCMatrix")
# mbrain_f <- as(mbrain_f, "dgCMatrix")
# 
# save(MALT_r,MALT_f,file = "~/Google Drive/Hallu/codes/ckgroup/DropletUtils/MALT.RData")
# save(mbrain_r,mbrain_f,file = "~/Google Drive/Hallu/codes/ckgroup/DropletUtils/mbrain.RData")
# PBMC1K_r <- as(PBMC1K_r, "dgCMatrix")
# PBMC1K_f <- as(PBMC1K_f, "dgCMatrix")
#save(PBMC1K_r,PBMC1K_f,file = "~/Google Drive/Hallu/codes/ckgroup/DropletUtils/PBMC1K.RData")


# MALT_p <- grep(pattern = "TotalSeqB",x = rownames(MALT_r))
# MALT_r <- MALT_r[-MALT_p,]
# 
# PBMC1K_p <- grep(pattern = "TotalSeqB",x = rownames(PBMC1K_r))
# PBMC1K_r <- PBMC1K_r[-PBMC1K_p,]

#load("~/Google Drive/Hallu/codes/ckgroup/DropletUtils/MALT_1_Ent_ts.RData")
#MALT_knee <- barcodeRanks(MALT_r,lower = 100)@metadata$knee
# MALT_inflection <- barcodeRanks(MALT_r,lower = 100)@metadata$inflection
#mbrain_knee <- barcodeRanks(mbrain_r,lower = 100)@metadata$knee
# mbrain_inflection <- barcodeRanks(mbrain_r,lower = 100)@metadata$inflection
# PBMC1K_knee <- barcodeRanks(PBMC1K_r,lower = 100)@metadata$knee
# PBMC1K_inflection <- barcodeRanks(PBMC1K_r,lower = 100)@metadata$inflection
# PBMC_knee <- barcodeRanks(PBMC_r,lower = 100)@metadata$knee
# PBMC_inflection <- barcodeRanks(PBMC_r,lower = 100)@metadata$inflection
# UW_knee <- barcodeRanks(UW_r,lower = 100)@metadata$knee
# UW_inflection <- barcodeRanks(UW_r,lower = 100)@metadata$inflection
...
SIM2 <-
  function(n = 5,
    ...
    brank <- DropletUtils::barcodeRanks(dat, lower = threshold)
    inflection <-
    ...
    for (sss in 1:n) {
      ...
      if (threegroup) {
        ...
        G2 <- as(G2, "dgCMatrix")
        G2 <- DropletUtils::downsampleMatrix(G2, 0.5)
      ...
      G3 <- as(G3, "dgCMatrix")
      G3 <- DropletUtils::downsampleMatrix(G3, 0.1)
      ...
      eOut <- DropletUtils::emptyDrops(dat_sim1, lower = threshold)
...
set.seed(SEED)
MALT_SIM <- SIM2(dat = MALT_r,dat_bg = MALT_bg, new_sim = F,Ncores=4)   #~15min v.s. ~3.5min
...
set.seed(SEED)
PBMC1K_SIM <- SIM2(dat = PBMC1K_r,dat_bg = PBMC1K_bg,new_sim = F,Ncores=4)  #~20min v.s. ~2.5min

set.seed(SEED)
PBMC_SIM <- SIM2(dat = PBMC_r,dat_bg = PBMC_bg,new_sim = F,Ncores=4) #~35min v.s. ~3.5min
  ######continue here
set.seed(SEED)
UW_SIM <- SIM2(dat = UW_r,dat_bg = UW_bg,new_sim = F,Ncores=4) #~24min v.s. ~6min

set.seed(SEED)
PBMC8K_SIM <- SIM2(dat = PBMC8K_r,dat_bg = PBMC8K_bg,new_sim = F,Ncores=4) #~24min v.s. ~6min
